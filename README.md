...
**Description:** This project is a music application with four main utilites: an Interval Ear Training Page, Tuner, Harmonizer and Metronome. The project uses the proccessing sound library in order to play a variety of pitches through piano sound files. The Interval Ear Training Page selects a random interval and scale degree and plays the interval. The user has to then guess the interval using an array of buttons and the program will respond with an indication of a correct or incorrect answer. The Tuner plays any given pitch based on user input through an array of buttons. The Harmonizer page is able to return a chord that harmonizes any given melody that the user inputs using a displayed score and the note class. The metronome repeatedly plays a sound file based on the frequency determined through user input. All of these pages are seperated and contained within their respective methods in the main file and converge within the mouseReleased and draw methods.  
