/* autogenerated by Processing revision 1307 on 2025-10-14 */
...
public class Caclulator extends PApplet {
...
public boolean isNums(String a) {
  try {
    Float.parseFloat(a);
    return true;
  }
...
public void setup() {
  ...
  cmdButtons[0] = new Button(100-20, 520, 100, 100, 0xFFCCB5E8, 0xFFDED3EB, "C");
  opButtons[0] = new Button(220-20, 520, 80, 100, 0xFF9FD2E3, 0xFFC1DDE6, "÷");
  opButtons[1] = new Button(340-30, 520, 80, 100, 0xFF9FD2E3, 0xFFC1DDE6, "X");
  opButtons[2] = new Button(460-40, 520, 80, 100, 0xFF9FD2E3, 0xFFC1DDE6, "-");
  opButtons[3] = new Button(580-50, 520, 80, 100, 0xFF9FD2E3, 0xFFC1DDE6, "+");
  for (int counter=1; counter<10; counter++) {
    int Ycounter=floor(counter*0.33f);
    numButtons[counter] = new Button(80+((140*(counter-1))-(Ycounter*420)), 640-10+(70*Ycounter), 100, 60, 0xFF9FD2E3, 0xFFC1DDE6, str(counter));
  }
  opButtons[4] = new Button(100-20, 840, 100, 60, 0xFF9FD2E3, 0xFFC1DDE6, "±");
  numButtons[0] = new Button(100+140-20, 840, 100, 60, 0xFF9FD2E3, 0xFFC1DDE6, "0");
  opButtons[5] = new Button(100+140*2-20, 840, 100, 60, 0xFF9FD2E3, 0xFFC1DDE6, ".");
  cmdButtons[1] = new Button(580-60+20, 630, 140, 100, 0xFF456145, 0xFF648264, "Graph");
  opButtons[6] = new Button(580-60, 720-6, 100, 60, 0xFF456145, 0xFF648264, "x");
  opButtons[7] = new Button(580-60, 720+60-3, 100, 60, 0xFF456145, 0xFF648264, "^");
  opButtons[9] = new Button(580-60, 720+60*2, 100, 60, 0xFF456145, 0xFF648264, "√");
  cmdButtons[2] = new Button(760-90+10, 630, 120, 100, 0xFF456145, 0xFF648264, "Window");
  opButtons[8] = new Button(760-90, 630+90, 140, 60, 0xFF456145, 0xFF648264, "log");
  cmdButtons[3] = new Button(760-90, 630+180, 140, 100, 0xFFCCB5E8, 0xFFDED3EB, "EXE");
}
...
public void updateDisplay() {
  ...
  if (negative==true) {
    if (vals.size()==1) {
      vals.set(0, str(PApplet.parseInt(vals.get(0))*(-1)));
    } else if (vals.size()>1) {
      for (int i=vals.size()-1; i>-1; i--) {
        if (Float.isNaN(PApplet.parseFloat(vals.get(i)))&&vals.get(i)!=".") {
          vals.set(i+1, str(PApplet.parseInt(vals.get(i+1))*(-1)));
          break;
        } else if (i==0) {
          vals.set(0, str(PApplet.parseInt(vals.get(0))*(-1)));
        }
  ...
  if (window==true) {
    ...
    if (windowCount==0&&windowCount<1) {
      ...
      xMax=PApplet.parseFloat(xPlaceh)/2;
      xMin=(PApplet.parseFloat(xPlaceh)/2)*(-1);
      yMax=PApplet.parseFloat(yPlaceh)/2;
      yMin=(PApplet.parseFloat(yPlaceh)/2)*(-1);
      windowCount=0;
      windowToggle*=(-1);
      window=false;
      cmdMonitor="C";
...
public void performCalculation() {
  ...
  for (int i=0; i<vals.size(); i++) {
    if (Float.isNaN(PApplet.parseFloat(vals.get(i)))&&vals.get(i)!="."&&vals.get(i)!="x") {
      ...
      tOp++;
      firstDigit=true;
    } else if (!Float.isNaN(PApplet.parseFloat(vals.get(i))) || vals.get(i).equals(".")||vals.get(i).equals("x")) {
      if (tOp==0) {
        if (firstDigit==true) {
  ...
  for (int i=0; i<tOp; i++) {
    for (int i2=0; i<(standardCalc.size())-1; i2++) {
      if (standardCalc.get(i2).equals("log")) {
        minusI=i2-1;
        plusI=i2+1;
        standardCalc.set(i2, str((log(PApplet.parseFloat(standardCalc.get(plusI))))/(log(PApplet.parseFloat(standardCalc.get(minusI))))));
        println(i, PApplet.parseFloat(standardCalc.get(minusI))*PApplet.parseFloat(standardCalc.get(plusI)));
    ...
    for (int i2=0; i<(standardCalc.size())-1; i2++) {
      if (standardCalc.get(i2).equals("^")) {
        minusI=i2-1;
        plusI=i2+1;
        standardCalc.set(i2, str(pow(PApplet.parseFloat(standardCalc.get(minusI)), PApplet.parseFloat(standardCalc.get(plusI)))));
        println(i, str(pow(PApplet.parseFloat(standardCalc.get(minusI)), PApplet.parseFloat(standardCalc.get(plusI)))));
        standardCalc.remove(plusI);
        standardCalc.remove(minusI);
        println(standardCalc);
        break;
    ...
    for (int i2=0; i<(standardCalc.size())-1; i2++) {
      if (standardCalc.get(i2).equals("√")) {
        plusI=i2+1;
        standardCalc.set(i2, str(sqrt(PApplet.parseFloat(standardCalc.get(plusI)))));
        println(i, str(sqrt(PApplet.parseFloat(standardCalc.get(minusI)))));
        standardCalc.remove(plusI);
        println(standardCalc);
        break;
      }
    ...
    for (int i2=0; i<(standardCalc.size())-1; i2++) {
      if (standardCalc.get(i2).equals("X")) {
        minusI=i2-1;
        plusI=i2+1;
        standardCalc.set(i2, str(PApplet.parseFloat(standardCalc.get(minusI))*PApplet.parseFloat(standardCalc.get(plusI))));
        println(i, PApplet.parseFloat(standardCalc.get(minusI))*PApplet.parseFloat(standardCalc.get(plusI)));
        standardCalc.remove(plusI);
        standardCalc.remove(minusI);
        println(standardCalc);
...
public void graphCalc() {
  ...
  for (x=xMin; x<xMax+0.01f; x=x+((abs(xMin)+abs(xMax))/148)) {
    xVals.append(x);
    ...
    lineVals.append(PApplet.parseFloat(standardCalc.get(0)));
  }
...
class Button {
  ...
  //constructor
  Button(int x, int y, int w, int h, int c1, int c2, String val) {
    this.x=x;
    this.y=y;
    this.w=w;
    this.h=h;
  ...
  public void clicked(int tempX, int tempY) {
    if (x>tempX-(w/2) && x<tempX+(w/2) && y>tempY-h/2 && y<tempY+h/2) {
      clicked=true;
    } else {
      clicked=false;
  ...
  public void settings() { size(770, 900); }

  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Caclulator" };
    if (passedArgs != null) {
